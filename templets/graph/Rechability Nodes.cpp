#include <bits/stdc++.h>
// given directed graph for each node count how many nodes it can reach
using namespace std;
void HOKSHA() {
...
signed main() {
    ...
    int sum[n + 1]{};
    vector<bitset<50005>>dp(graph.comp_cnt);
    ...
    vector<int>topo;
    vector<int>vis(graph.comp_cnt, 0);
    function<void(int)> solve = [&](int  comp)-> void{
        vis[comp] = 1;
    ...
    for(auto j : topo){
        for(auto cp : graph.condensed[j]){
            dp[j] |= dp[cp];
        }
    }
    for(int i = 1; i <= n; i++){
        cout << dp[graph.comp_id[i]].count() << ' ';
    }
