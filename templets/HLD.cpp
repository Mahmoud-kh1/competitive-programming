...
//    cin >> t;
    while(t--){
        ...
        for(auto [u, v, w] : edges){
            if (hd.depth[u] > hd.depth[v]){
                int id = hd.idchain[u];
                seg[id].update(hd.pos[u], w);
            }
            else
            {
                int id = hd.idchain[v];
                seg[id].update(hd.pos[v], w);
            }
        }
        ...
        while(q--){
            ...
            if (ty == 1){
                ...
                if (hd.depth[u] > hd.depth[v]){
                    int id = hd.idchain[u];
                    seg[id].update(hd.pos[u], x);
                }
                else
                {
                    int id = hd.idchain[v];
                    seg[id].update(hd.pos[v], x);
                }
            }
            else{
                int u, v; cin >> u >> v;
                ...
                while(true){
                    if (hd.head[u] == hd.head[v]){
                        if (hd.depth[v] < hd.depth[u]) swap(u,v);
                        res = (res + seg[hd.idchain[u]].query(hd.pos[u] + 1, hd.pos[v]));
                        break;
                    }
                    if (hd.depth[hd.head[u]] > hd.depth[hd.head[v]]) swap(u, v);
                    res = (res + seg[hd.idchain[v]].query(hd.pos[hd.head[v]], hd.pos[v]));
                    v =hd.par[hd.head[v]];
                }
                cout << res << '\n' ;
            }
