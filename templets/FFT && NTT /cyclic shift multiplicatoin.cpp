// f = sum of a[i] * b[i] and we are given query in each we do shift right to b by k step and output the sum 
...
cin >> t;
    while(t--) {
        ...
        vector<int>pa(2 * n), pb(2 * n);

        // a1, a2, a3
        // b1, b2, b3

        // so if we make one right shift for b a1 must be multiply by b3
        // we double b and extend a with zeors  0 0 0 a1 a2 a3 , b1 b2 b3 b1 b2 b3

        // if shift left we make it like a1, a2 a3 0 0 0 , b1 b2 b3 b1 b2 b3 and query with -shifts
        for(int i = 0; i < n; i++){
            cin >> a[i + n];
            pa[i + n] = a[i + n];
        }
