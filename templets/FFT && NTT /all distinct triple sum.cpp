...
//cin >> t;
    while(t--) {
        ...
        int sh = 20001;
        vector<int>triple(mx + sh + 1);
        vector<int>pa(mx + sh + 1);
        for(int i = 0; i < n; i++){
            triple[a[i] + sh]++;
            pa[a[i] + sh]++;    
        }
        triple = multiply(pa, pa);
        triple = multiply(triple, pa);

        vector<int>pb(2*mx + 2* sh + 1);
        for(int i = 0; i < n; i++) pb[2*a[i] + 2 * sh]++;
        vector<int>res = multiply(pb, pa);
        for(int i= 0; i < n; i++){
            res[a[i] + a[i] + a[i] + 3 * sh]--;
        }
        for(int i = 0; i < n; i++) triple[a[i] + a[i] + a[i] + 3 * sh]--;

        for(int i = -3*mx; i <= 3*mx; i++){
            triple[i + 3 * sh] -= res[i + 3 * sh] * 3;
            if(triple[i + 3  * sh] >0){
                cout << i << " : " << triple[i +3 * sh] / 6 << endl;
            }
        }
    }
